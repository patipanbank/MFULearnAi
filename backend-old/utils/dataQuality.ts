 ...
 */
export function validateChunk(chunk: string, minLength: number = 10): ChunkValidationResult {
 ...
 */
export function removeNearDuplicates(chunks: string[], similarityThreshold: number = 0.9): string[] {
  if (chunks.length <= 1) {
    return chunks;
  }
 ...
 */
export function validateAndNormalizeChunks(chunks: string[], shouldRemoveNearDuplicates: boolean = true): {
  validChunks: string[];
  invalidCount: number;
  errors: string[];
} {
 ...
 */
export function validateExtractedText(text: string, minLength: number = 100): ChunkValidationResult {
  if (!text || typeof text !== 'string') {
    return {
      isValid: false,
