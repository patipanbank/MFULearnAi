...
router.put('/:collectionId', async (req, res) => {
    ...
    const updated = await collectionService_1.collectionService.updateCollection(collectionId, updates, user.username);
    res.json(updated);
    return null;
});
router.delete('/:collectionId', async (req, res) => {
    ...
    const ok = await collectionService_1.collectionService.deleteCollection(collectionId, user.username);
    if (ok) {
        res.status(204).send();
        return null;
    }
