  ...
  async getRelevantContext(sessionId: string, query: string, messageCount: number): Promise<ChatMessage[]> {
    ...
    switch (strategy) {
      case 'vector':
        ...
        // Convert Document[] to ChatMessage[]
        return docs.map(doc => ({
  ...
  async addMessage(sessionId: string, message: ChatMessage, messageCount: number): Promise<void> {
    try {
      ...
      if (message.role === 'user') {
        await this.redisHistoryService.addMessage(sessionId, new HumanMessage(message.content));
      } else if (message.role === 'assistant') {
        await this.redisHistoryService.addMessage(sessionId, new AIMessage(message.content));
      }
