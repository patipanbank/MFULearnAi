...
export class StatsService {
  ...
  private async getTokenUsageAnalytics(start: Date, end: Date): Promise<any> {
    try {
      // Mock token usage data (in production, use real token tracking)
      const tokenUsage = await this.chatModel.aggregate([
        { $match: { createdAt: { $gte: start, $lte: end }, isActive: true } },
        {
          $group: {
            ...
            totalTokens: { $sum: { $multiply: [{ $size: '$messages' }, 150] } }, // Approximate
            requestCount: { $sum: 1 },
          },
        },
        { $sort: { '_id.date': 1 } },
      ...
      return {
        totalTokens: tokenUsage.reduce((sum, item) => sum + item.totalTokens, 0),
        totalRequests: tokenUsage.reduce((sum, item) => sum + item.requestCount, 0),
        averageTokensPerRequest: tokenUsage.length > 0 ? 
          tokenUsage.reduce((sum, item) => sum + item.totalTokens, 0) / 
          tokenUsage.reduce((sum, item) => sum + item.requestCount, 0) : 0,
        dailyBreakdown: tokenUsage,
      };
    } catch (error) {
      console.error('Error getting token usage analytics:', error);
      return { totalTokens: 0, totalRequests: 0, averageTokensPerRequest: 0, dailyBreakdown: [] };
    }
  }
  ...
  private async getModelUsageAnalytics(start: Date, end: Date): Promise<any> {
    try {
      ...
      return modelUsage.map(item => ({
        ...
        percentage: modelUsage.length > 0 ? 
          ((item.usage_count / modelUsage.reduce((sum, m) => sum + m.usage_count, 0)) * 100).toFixed(2) : 0,
      }));
    } catch (error) {
  ...
  private async getDepartmentAnalytics(start: Date, end: Date): Promise<any> {
    try {
      ...
      return departmentUsage.map(item => ({
        department: item.department || 'unknown',
        chat_count: item.chat_count,
        message_count: item.message_count,
        unique_users: item.unique_users,
        average_messages_per_chat: item.chat_count > 0 ? 
          Math.round((item.message_count / item.chat_count) * 100) / 100 : 0,
      }));
    } catch (error) {
  ...
  private async getPerformanceMetrics(start: Date, end: Date): Promise<any> {
    try {
      // Mock performance metrics (in production, use real performance tracking)
