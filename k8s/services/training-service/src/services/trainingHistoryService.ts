...
export class TrainingHistoryService {
  ...
  async deleteHistoryByCollection(collectionName: string): Promise<number> {
    try {
      ...
      console.log(`Deleted ${result.deletedCount} training history records for collection: ${collectionName}`);
  ...
  async deleteHistoryByDocument(collectionName: string, documentName: string): Promise<number> {
    try {
      ...
      console.log(`Deleted ${result.deletedCount} training history records for document: ${documentName}`);
      return result.deletedCount || 0;
    } catch (error) {
