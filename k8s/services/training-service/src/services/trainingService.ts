...
export class TrainingService {
  ...
  private async embedAndStore(
    ...
    try {
      ...
      // Create embeddings using RAG service via HTTP
      ...
      while (retryCount < maxRetries) {
        try {
          const response = await axios.post(
            `${config.RAG_SERVICE_URL}/api/embedding/batch`,
            { texts: chunks },
          ...
          retryCount++;
          console.error(`Embedding attempt ${retryCount} failed:`, embeddingError.message);
      ...
      if (documentsToAdd.length > 0) {
        console.log(`Storing ${documentsToAdd.length} documents in ChromaDB via RAG service...`);

        // Store documents in ChromaDB via RAG service
        await axios.post(
          `${config.RAG_SERVICE_URL}/api/chroma/add`,
          {
            collectionName,
  ...
  async createEmbeddingsWithProgress(
    ...
    while (retryCount < maxRetries) {
      try {
        onProgress?.(retryCount * 33);

        // Call RAG service for embeddings
        const response = await axios.post(
          `${config.RAG_SERVICE_URL}/api/embedding/batch`,
          { texts: chunks },
          { timeout: 120000 }
    ...
    try {
      ...
      if (numChunks > 0) {
        try {
          ...
          console.warn('Failed to record training history:', historyError);
