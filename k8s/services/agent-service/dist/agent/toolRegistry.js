...
const httpClients_1 = require("../services/httpClients");
...
exports.toolRegistry = {
    ...
    memory_search: {
        ...
        func: async (input, sessionId) => {
            ...
            const results = await httpClients_1.ragClient.searchMemory(sessionId, input);
            if (!results.length)
    ...
    memory_embed: {
        ...
        func: async (input, sessionId) => {
            if (!sessionId)
                return 'No sessionId provided.';
            await httpClients_1.ragClient.embedMessage(sessionId, input);
...
function createMemoryTool(sessionId) {
    return {
        ...
        [`recent_context_${sessionId}`]: {
            ...
            func: async () => {
                try {
                    const recent = await httpClients_1.ragClient.searchMemory(sessionId, '', 10);
                    if (!recent.length)
        ...
        [`full_context_${sessionId}`]: {
            ...
            func: async () => {
                try {
                    const all = await httpClients_1.ragClient.searchMemory(sessionId, '', 100);
                    if (!all.length)
        ...
        [`clear_memory_${sessionId}`]: {
            ...
            func: async () => {
                try {
                    ...
                    console.log('Memory clear requested for session:', sessionId);
        ...
        tools[`search_${collectionName}`] = {
            ...
            func: async (input) => {
                try {
                    ...
                    const results = await httpClients_1.ragClient.searchCollection(collectionName, input, 5);
...
async function addChatMemory(sessionId, messages) {
    for (const msg of messages) {
        await httpClients_1.ragClient.embedMessage(sessionId, msg.content);
    }
}
