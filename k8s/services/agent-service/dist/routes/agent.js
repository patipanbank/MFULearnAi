...
const langGraphExecutionService_1 = require("../services/langGraphExecutionService");
...
// Get agent by ID
router.get('/:agentId', extractUser, async (req, res) => {
    try {
...
router.post('/', extractUser, async (req, res) => {
    try {
        ...
        const agent = await agentService_1.agentService.createAgent(agentData, req.user);
...
// Update agent
router.put('/:agentId', extractUser, async (req, res) => {
    try {
        const { agentId } = req.params;
        const userId = req.user.sub;
        const updates = req.body;
        const agent = await agentService_1.agentService.updateAgent(agentId, updates, userId);
        if (!agent) {
            return res.status(404).json(null);
        }
        return res.json(agent);
...
// Delete agent
router.delete('/:agentId', extractUser, async (req, res) => {
    try {
        const { agentId } = req.params;
        const userId = req.user.sub;
...
// Get agent templates
router.get('/templates/all', extractUser, async (req, res) => {
    try {
...
router.post('/templates/:templateId', extractUser, async (req, res) => {
    try {
...
// Get tool statistics
router.get('/tools/statistics', extractUser, async (req, res) => {
    try {
        const user = req.user;
...
router.post('/execute', extractUser, async (req, res) => {
    try {
        ...
        await langGraphExecutionService_1.langGraphExecutionService.executeAgent(executionRequest, (event) => {
            try {
                if (event.type === 'chunk') {
