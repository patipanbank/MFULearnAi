...
// SAML Callback (POST) - Main callback handler
router.post('/saml/callback', (req: Request, res: Response, next: NextFunction) => {
  passport.authenticate('saml', async (err: any, profile: any, info: any) => {
    ...
    try {
      // Enhanced SAML attributes mapping
      console.log('\n🔍 Raw SAML Attributes:');
      for (const [key, value] of Object.entries(samlAttributes)) {
        console.log(`   ${key}: ${JSON.stringify(value)}`);
      }
      ...
      const getAttr = (keyArr: string[], fallback?: any) => {
        for (const key of keyArr) {
          if (samlAttributes[key] && Array.isArray(samlAttributes[key]) && samlAttributes[key][0]) {
            // For Group SIDs, return the entire array
      ...
      // Improved groups mapping to match backend-legacy
      let groups = [];
      const groupsAttr = getAttr(['Groups']); // Use Groups string attribute
      const groupSids = getAttr(['http://schemas.xmlsoap.org/claims/Group']); // Use Group SIDs array
      ...
      // Use Group SIDs as primary, same as backend-legacy
      ...
      // Create user profile and save to DB
      const userProfile = {
        ...
        lastName,
        department,
        groups: groupsArray, // Store SID array
      };
...
router.post('/admin/login', async (req: Request, res: Response) => {
  ...
  // Create JWT payload same as Python
  const tokenPayload = {
    sub: user._id,
    nameID: user.nameID,
    username: user.username,
...
router.post('/refresh', authenticateJWT, requireAnyRole, (req: Request, res: Response) => {
  ...
  // Create new token (7 days) same as Python
