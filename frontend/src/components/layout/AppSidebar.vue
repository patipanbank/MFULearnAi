  ...
  <aside class="app-sidebar" :class="{ 'collapsed': isCollapsed }">
    ...
    <div class="sidebar-footer">
      <button class="settings-trigger-btn" @click="showSettings = true" :class="{ 'collapsed': isCollapsed }">
        <div class="user-avatar-small">{{ authStore.displayName?.charAt(0)?.toUpperCase() }}</div>
        <span class="settings-text" v-if="!isCollapsed">Settings & Menu</span>
        <svg v-if="!isCollapsed" class="chevron-icon" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="18 15 12 9 6 15"></polyline></svg>
      </button>
    </div>
