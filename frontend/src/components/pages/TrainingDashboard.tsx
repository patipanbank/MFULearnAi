    ...
    try {
      ...
      const response = await fetch(`${config.apiUrl}/api/training/add-urls`, {
        ...
        body: JSON.stringify({
          ...
          crawlDepth: crawlDepth // เพิ่มตัวแปรนี้
        }),
      });
      ...
      alert(`ประมวลผล URL เสร็จสิ้น\nสำเร็จ: ${result.results.filter((r: any) => r.success).length}\nล้มเหลว: ${result.results.filter((r: any) => !r.success).length}`);
      
      setUrlInput('');
      fetchUploadedFiles(selectedCollection?.name || '');
    } catch (error) {
      console.error('Error submitting URLs:', error);
      alert(error instanceof Error ? error.message : 'Failed to process URLs');
    }
  };
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col space-y-4">
            <div className="flex items-center gap-4">
              <form onSubmit={onFileUpload} className="flex-1">
                ...
                <div className="flex items-center gap-4">
                  <div>
                    ...
                    <input
                      ...
                      min={1}
                      max={5}
                      className="w-20 p-2 border rounded-lg dark:bg-gray-700 
                        dark:border-gray-600 dark:text-gray-200"
                    />
                  </div>
                  <button
