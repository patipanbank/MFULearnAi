...
const UserProfile: React.FC = React.memo(() => {
  ...
  ), [handleToggleDropdown, getInitials, user.firstName, user.lastName, user.student_id]);
  ...
  const dropdownContent = useMemo(() => {
    ...
    return (
      <div className="absolute right-0 mt-2 w-[280px] md:w-[350px] bg-primary border border-border rounded-xl shadow-xl overflow-hidden animate-slide-in-from-bottom-2 duration-200">
        {/* User Profile Header */}
        <div className="p-3 md:p-4 bg-gradient-to-r from-[rgb(186,12,47)] to-[rgb(212,175,55)] animate-in fade-in duration-300 delay-75">
          <div className="flex items-center space-x-3 md:space-x-4">
            <div className="h-12 w-12 md:h-16 md:w-16 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center text-white text-xl md:text-2xl font-semibold animate-in fade-in zoom-in duration-300 delay-150">
              {getInitials()}
            </div>
        ...
        <div className="p-3 md:p-4 space-y-4 md:space-y-5">
          ...
          <div className="flex items-center space-x-3 md:space-x-4 animate-in fade-in slide-in-from-right-3 duration-300 delay-200">
            <div className="w-5 h-5 md:w-6 md:h-6 flex items-center justify-center bg-blue-100 rounded-lg">
            ...
            <div className="flex-1">
              <div className="text-xs md:text-sm text-muted">EMAIL</div>
              <div className="text-sm md:text-base text-primary">{user.email}</div>
            </div>
          </div>

          {/* Department */}
          <div className="flex items-center space-x-3 md:space-x-4 animate-in fade-in slide-in-from-right-3 duration-300 delay-300">
            <div className="w-5 h-5 md:w-6 md:h-6 flex items-center justify-center bg-green-100 rounded-lg">
          ...
          {/* Groups */}
          <div className="flex items-center space-x-3 md:space-x-4 animate-in fade-in slide-in-from-right-3 duration-300 delay-[400ms]">
            <div className="w-5 h-5 md:w-6 md:h-6 flex items-center justify-center bg-amber-100 rounded-lg">
            ...
            <div className="flex-1">
              ...
              <div className="mt-0.5 md:mt-1">
                <span className="inline-flex items-center px-2 py-0.5 md:px-3 md:py-1 rounded-full text-xs md:text-sm font-medium bg-purple-100 text-purple-800">
                  {user.role}
                </span>
              </div>
            </div>
        ...
        {/* Sign Out Button */}
        <div className="p-3 md:p-4 border-t border-border animate-in fade-in slide-in-from-bottom-2 duration-300 delay-500">
          <button
            onClick={handleCompleteLogout}
            className="w-full flex items-center justify-center space-x-2 px-3 py-2 md:px-4 md:py-3 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition-colors font-medium text-sm md:text-base"
          >
  ...
  }, [isOpen, getInitials, user.firstName, user.lastName, user.username, user.student_id, user.email, user.role, handleCompleteLogout]);
